...
- ✅ **v0.1.8 — RSS kaynak yönetimi:** Settings → RSS Kaynakları editable editor (↑↓ sıra + 🔴 breaking toggle + ad + URL + ✓ feed + 🔔 bildirim + × sil + Yeni Ekle + Sıfırla). Auto-discover (`GET /api/discover?url=...` → `<link rel="alternate">` parse + fallback `/feed`,`/rss`,`/rss.xml`,`/atom.xml`,`/feed/`). Backend `sourcesState.js` runtime state (`PUT/DELETE /api/sources/runtime`), client her settings değişiminde push, RSS cache flush'ı `onChange` listener ile. `sources.json` schema: `category` kaldırıldı, `breaking: bool` eklendi (NTV/Cumhuriyet/Habertürk/CNN Türk → true). `BreakingMarquee` artık hardcoded değil — props ile dinamik breaking ID listesi alır.
- ✅ **v0.1.9 — Ticker iyileştirmeleri:** Animasyon hızı 80s → 55s. Settings → Borsa Sembolleri editor (↑↓ sıra + etiket + Yahoo sembolü + × sil + Yeni Ekle + Sıfırla). Backend yeni endpoint `GET /api/finance/defaults`. Client `customSymbols` settings field'ı (null=backend default, array=override). Ticker artık `symbols` prop alır, label client tarafında saklanır (eskiden backend `label=symbol` döndürüyordu). Eski `selectedSymbols` field migration ile silinir.
- ✅ **v0.1.10 — README + LICENSE + electron-updater:** README.md ürün sayfası tonunda yazıldı. LICENSE Custom Source-Available (flashcards-app paterni, repo adı güncel). electron-updater entegrasyonu: `electron/updater.cjs` autoUpdater wrapper (autoDownload off, ilk check 30s, sonra saatlik), main.cjs IPC handlers (check/download/quit-install/is-active), preload event subscription (`onUpdateStatus`). VersionTag.jsx state machine refactor: idle → checking → available (İndir butonu) → downloading (progress bar) → downloaded (Yeniden Başlat butonu). package.json `publish` config (github provider). Eski GitHub Releases API manuel check kodu kaldırıldı.
- ✅ **v0.2.0 — Tema motoru (v1.3 alt-paket 1/4):** `client/src/themes.js` 8 preset havuzu (Klasik/Yayıncı/Finans/Terminal/Sade/Creative/Sıcak/Camgöbeği) — her preset 7 CSS değişkeni override eder (`--bg`, `--bg-2`, `--bg-3`, `--border`, `--text`, `--text-dim`, `--accent`). `applyTheme(id)` runtime'da `documentElement.style.setProperty` ile uygular + `data-theme` attribute. App.jsx defaults `theme: 'classic'`, settings effect'inde apply. Settings → "🎨 Görünüm" section: 4 sütun grid chip selector, her chip kendi accent rengini yansıtır. Sıradaki sub-paketler: v0.2.1 custom HEX picker, v0.2.2 font seçimi, v0.2.3 marquee/ticker mikro ayarlar.
- ✅ **v0.2.1 — Custom accent renk (v1.3 alt-paket 2/4):** `settings.customAccent: null | "#hex"` — preset üzerine accent override. `applyTheme(id, customAccent)` opsiyonel parametre; varsa `--accent`'i o değer üzerine yazar. Settings → Görünüm: preset chip'lerinin altına color picker satırı (HTML5 native `<input type="color">`), hex değer rozeti ve "↺ Sıfırla" butonu. Preset değişimi customAccent'ı `null`'a çevirir (clean slate UX).
- ✅ **v0.2.2 — Font seçimi (v1.3 alt-paket 3/4):** `client/src/fonts.js` 12 seçenek (7 sistem + 5 bundled web). Bundled fontlar `@fontsource/*` paketleri ile (Inter, Source Sans 3, Manrope, Merriweather, JetBrains Mono) — Regular + Bold weight, offline-friendly. `applyFont(id)` `--font-family` CSS değişkenini set eder; `body { font-family: var(--font-family, ...) }`. `settings.fontFamily: 'system'` defaults. Settings → Görünüm: `<select>` dropdown (Sistem/Web optgroup), "↺ Sıfırla" butonu, canlı preview metni.
- ✅ **v0.2.3 — Marquee/Ticker mikro ayarlar (v1.3 alt-paket 4/4):** Yeni CSS değişkenleri `--marquee-{bg,fg,duration,font}` ve `--ticker-{bg,fg,duration,font}` (`:root`'ta default'lar). `settings.marquee = {bg,fg,speed,font}` ve `settings.ticker = {...}` (null = default). App.jsx `applyStripStyles(prefix, opts)` helper'ı runtime'da `setProperty/removeProperty`. Settings → "🎬 Şerit Ayarları" section: iki `StripEditor` (Son Dakika / Borsa), her birinde 4 kontrol (arka plan/yazı color picker + hız number input 20-180s + font select). Her alan ayrı sıfırlanabilir. v1.3 tema motoru paketi tamamlandı.
